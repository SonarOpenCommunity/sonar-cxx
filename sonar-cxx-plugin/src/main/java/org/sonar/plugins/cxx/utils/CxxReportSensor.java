...
public abstract class CxxReportSensor implements Sensor {
     ...
     *
     * @param conf
     *            the Settings object used to access the configuration
     *            properties
     * @param fs
     ...
     *            file system access layer
     * @param metric
     *            this metrics will be used to save a measure of the overall
     *            issue count. Pass 'null' to skip this.
     */
    protected CxxReportSensor(ResourcePerspectives perspectives, Settings conf,
	    ModuleFileSystem fs, Metric metric) {
	...
	try {
	    List<File> reports = getReports(conf, fs.baseDir().getPath(), reportPathKey(),
		    defaultReportPath());
		...
		try {
		    ...
		    CxxUtils.LOG.info("{} processed = {}",
			    metric == null ? "Issues" : metric.getName(), violationsCount
				    - prevViolationsCount);
	...
	if (uniqueIssues.add(file + line + ruleId + msg)) { // StringBuilder is
							    // slower
	...
	if ((filename != null) && (filename.length() > 0)) { // file level
		...
		org.sonar.api.resources.File file = org.sonar.api.resources.File.fromIOFile(
			new File(normalPath), project);
	...
	if (issuable != null) {
	    Issuable.IssueBuilder issueBuilder = issuable.newIssueBuilder().ruleKey(rule)
		    .message(msg);
	    if (lineNr > 0) {
