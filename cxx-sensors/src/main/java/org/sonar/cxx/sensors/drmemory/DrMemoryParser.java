...
public class DrMemoryParser {
  ...
  public enum DrMemoryErrorType {
    UNADRESSABLE_ACCESS("UnadressableAccess", "UNADDRESSABLE ACCESS"), UNINITIALIZE_READ("UninitializedRead",
        "UNINITIALIZED READ"), INVALID_HEAP_ARGUMENT("InvalidHeapArgument",
            "INVALID HEAP ARGUMENT"), GDI_USAGE_ERROR("GdiUsageError", "GDI Usage Error"), HANDLE_LEAK("HandleLeak",
                "HANDLE LEAK"), WARNING("DrMemoryWarning", "WARNING"), POSSIBLE_LEAK("PossibleMemoryLeak",
                    "POSSIBLE LEAK"), LEAK("MemoryLeak", "LEAK"), UNRECOGNIZED("Dr Memory unrecognized error", "");
  ...
  public static List<DrMemoryError> parse(File file, String string) {

    List<DrMemoryError> result = new ArrayList<>();

    List<String> elements = getElements(file, string);
